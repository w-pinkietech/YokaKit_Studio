...
### テンプレート整備フロー
> Claude Code のインストールや同期方法は [studio/50-tooling/claude/README.md](../../50-tooling/claude/README.md) を参照。
...
3. テンプレートに追加の入力が必要な場合は、`.aidlc/contexts/` や `studio/` にある関連ドキュメントから参照できるよう導線を張る。  
...
> `.claude/commands/` の更新は必ず Issue を起点に行い、README / `studio/20-process/agents/README.md` / 本ガバナンス文書の記述を同期させること。
...
- ドキュメントは `studio/` ディレクトリに集約し、ヒューマン向け・AI向けガイド（`studio/README.md`, `studio/20-process/agents/README.md`）を随時更新する。
